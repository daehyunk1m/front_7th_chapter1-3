...
디자인 시스템이나 UI 컴포넌트 라이브러리에 관심이 많아 컴포넌트를 관리하는 '스토리북'은 들어봤지만 테스트와 관련된 분야라는 것은 알지 못했다.
playwright도 mcp나 컨퍼런스의 주제 등 어디선가 보던 타이틀이지만 내가 궁금해 하던 테스트와 관련 있을거라곤 생각하지 못했다.
이번 주차까지 3주간 과제를 통해 테스트 까막눈에서 이제는 알파벳은 읽을 수 있는 실력이 된 것 같다.
단위테스트와 통합 테스트의 장단점과 차이점을 이해하고 E2E 테스트와 시각적 회귀 테스트를 통해 프론트엔드 개발자로서 비즈니스에 적극적으로 기여할 수 있다는 자신감을 얻을 수 있었다.
